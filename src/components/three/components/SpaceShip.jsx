...
import { createAttractor, lorenzMod2Attractor, updateAttractor } from '../utils/attractor'
...
export function SpaceShip(props) {
  ...
  const [positions, currentPosition] = useMemo(() => createAttractor(30, [0, RADIUS, 0]), [])

  useFrame(() => {
    if (line.current) {
      const target = updateAttractor(currentPosition, RADIUS, lorenzMod2Attractor, 0.002)

      if (nextPosition) {
        group.current.position.copy(nextPosition.clone().add(new THREE.Vector3(0, 0, 0)))
  ...
  return (
    <>
      ...
      <mesh>
        ...
        <meshLineMaterial attach="material" lineWidth={0.25} color="#FCEEB5" transparent opacity={0.5} />
      </mesh>
    </>
  )
}
